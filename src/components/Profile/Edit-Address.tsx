...
const UpdateAddressForm: React.FC = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchClient({
        method: "PUT",
        url:
          process.env.NEXT_PUBLIC_BACKEND_API_URL + "/api/user/update-addresses",
        body: JSON.stringify(addressData),
      });
      ...
      router.push("/dashboard/setting"); // kembali ke halaman setting
        ...
        Swal.fire({
          position: "top",
          icon: "error",
          title: "Failed to address, please try again",
          showConfirmButton: false,
          timer: 1000,
        })
      }
  ...
  return (
    <div className="flex flex-col items-center rounded-lg border border-gray-200 p-6 shadow-md w-full max-w-lg bg-white mx-auto mt-5">
