    ...
    const fetcThreshold = async () => {
      try {
        const response = await fetchClient({
          method: "GET",
          url: `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/device-setting/${productId}`,
        });
  ...
  const handleSubmitTh = async () => {
    ...
    try {
      const response = await fetchClient({
        method: "PUT",
        url:
          `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/update-threshold/${productId}`,
        body: JSON.stringify({
          min_suhu: threshold1,
          max_suhu: threshold2,
        }),
  ...
  const handleControlFan = async (state: string) => {
    ...
    try {
      const response = await fetchClient({
        method: "PUT",
        url:
          `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/update-fan-status/${productId}`,
        body: JSON.stringify({
          fan: state,
        }),
      });
  ...
  const handleControlLamp = async (state: string) => {
    ...
    try {
      const response = await fetchClient({
        method: "PUT",
        url:
          `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/update-lampu-status/${productId}`,
        body: JSON.stringify({
          lampu: state,
        }),
      });
        ...
        <div className="p-4">
          ...
          <div className="bg-gray-200 rounded rounded-lg p-6">
            ...
            <div className="flex bg-gray-100 p-2 rounded-lg shadow-md text-sm">
              <div className="lg:space-x-4 space-x-1">
                <button
                  ...
                  disabled={loading}
                  className={`lg:px-6 px-4 lg:py-2 py-1 bg-green-500 text-white rounded-lg shadow-md hover:bg-green-600 transition duration-200 ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  Turn ON
                </button>
                <button
                  ...
                  disabled={loading}
                  className={`lg:px-6 px-4 lg:py-2 py-1 bg-red-500 text-white rounded-lg shadow-md hover:bg-red-600 transition duration-200 ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  Turn OFF
                </button>
                <button
                  ...
                  disabled={loading}
                  className={`lg:px-6 px-4 lg:py-2 py-1 bg-yellow-500 text-white rounded-lg shadow-md hover:bg-yellow-600 transition duration-200 ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  Turn AUTO
                </button>
              </div>
            ...
            {responseMessageLam && (
              <div
                className={`mt-4 p-2 text-black rounded-md ${
                  responseMessageLam.startsWith("Success") ? "bg-yellow-500" : "bg-red-500"
                }`}
              >
                {responseMessageLam}
              </div>
